...
// Tinys procedures
procedure loadArray ( var lD:integer; var arr:letters ; word:string);      // load array of char with '_'
var
    i: integer;
begin
    lD:=long(word);
...
begin
    for i:=1 to lD do
        array[i]:=word[i];  // select one 'char' per vector of the array of string
end;
...
procedure possibleChallenge (var totL:integer,var aux:boolena,var lostLifes:range2,lD:integer,var tries:integer);    
...
var
    ...
    lostLifes: range2;
    pictures: painter;
    tries,mode,logic_dim,totalLetters: integer;
begin
    ...
    repeat
        ...
        loadArray(logic_dim,arrayy);

        mainAxis(logic_dim,arrayy,word,show);
        writeln('You want to GO to the MAIN MENU? y/n')
        until ((end = 'n') or (end = 'N'))
...
procedure mainAxis (lD:integer;arr:letters;wrd:string;show:boolean);  //This is the most important PROCESS, the life of the program the MAIN AXIS!
var                                                                   
    usrL:char;
